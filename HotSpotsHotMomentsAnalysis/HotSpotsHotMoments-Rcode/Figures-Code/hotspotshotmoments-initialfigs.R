# hotspotshotmoments-initialfigs.R
# 
# make figures for the storyboard for the hot spots/hot moments paper
#
...
# output products:
# figures in folder /HotSpotsHotMoments-Figures-Analyses/
# tables in folder /HotSpotsHotMoments-Figures-Analyses/
...
pathsavefigs = "~/Documents/GITHUB/cso044code_HotSpotsHotMoments/HotSpotsHotMomentsAnalysis/HotSpotsHotMoments-Figures-Analyses/StoryboardFigures/"
...
soildatapath <- "~/Documents/GITHUB/cso044code_HotSpotsHotMoments/HotSpotsHotMomentsAnalysis/HotSpotsHotMoments-Data-Raw/Sensors/SurfaceOutOfDate/OldestCSOHas/renamedtomatchcode/"
pathsensorchambermatch = "~/Documents/GITHUB/cso044code_HotSpotsHotMoments/HotSpotsHotMomentsAnalysis/"
...
histCO2a <- ggplot(arrayGHGdf, aes(x=CO2_umolm2s_fluxuse)) + geom_histogram(aes(y=..density..), binwidth=0.5, colour="black", fill="white") + theme_bw() + facet_wrap( ~ ChamberNameSilver, ncol = 3) + ylab("Density Distribution") + xlab("CO2 Flux (umol/m^2/s)") + theme_set(theme_bw(base_size = 36)) # + geom_density(alpha=.2, fill="#FF6666")

png(file = paste(pathsavefigs, "CO2_densityplots.png", sep=""),width=10,height=10,units="in",res=400)
histCO2a
dev.off()
...
png(file = paste(pathsavefigs, "CO2_densityplots_combo.png", sep=""),width=5,height=5,units="in",res=400)
histCO2b
dev.off()

# Histogram overlaid with kernel density curve, faceting by topo location
histCO2c <- ggplot(arrayGHGdf, aes(x=CO2_umolm2s_fluxuse)) + geom_histogram(aes(y=..density..), binwidth=.5, colour="black", fill="white") + facet_wrap(System ~ Topo, ncol = 3) + geom_density(alpha=.2, fill="#FF6666") #+ xlim(c(-0.5,0.75)) 
# histogram with density instead of count on y-axis
# overlay with transparent density plot
# long tail isn't too bad for visualization

png(file = paste(pathsavefigs, "CO2_densityplots_topo.png", sep=""),width=10,height=5,units="in",res=400)
histCO2c
dev.off()
...
# Histogram overlaid with kernel density curve, fluxes by chamber
histCH4a <- ggplot(arrayGHGdf, aes(x=CH4_nmolm2s_fluxuse)) + geom_histogram(aes(y=..density..), binwidth=2, colour="black", fill="white") + theme_bw() + facet_wrap( ~ ChamberNameSilver, ncol = 3) + ylab("Density Distribution") + xlab("CH4 Flux (nmol/m^2/s)") + theme_set(theme_bw(base_size = 36)) + xlim(c(-25,50)) + ylim(c(0,0.35)) #+ geom_density(alpha=.2, fill="#FF6666")
# histogram with density instead of count on y-axis
# overlay with transparent density plot
# without limiting the axes the long tail made things too hard to read

png(file = paste(pathsavefigs, "CH4_densityplots.png", sep=""),width=10,height=10,units="in",res=400)
histCH4a
dev.off()
...
png(file = paste(pathsavefigs, "CH4_densityplots_combo.png", sep=""),width=5,height=5,units="in",res=400)
histCH4b
dev.off()

# Histogram overlaid with kernel density curve, faceting by topo location
histCH4c <- ggplot(arrayGHGdf, aes(x=CH4_nmolm2s_fluxuse)) + geom_histogram(aes(y=..density..), binwidth=1, colour="black", fill="white") + facet_wrap(System ~ Topo, ncol = 3) + geom_density(alpha=.2, fill="#FF6666") + xlim(c(-25,50))
# histogram with density instead of count on y-axis
# overlay with transparent density plot
# without limiting the axes the long tail made things too hard to read

png(file = paste(pathsavefigs, "CH4_densityplots_topo.png", sep=""),width=10,height=5,units="in",res=400)
histCH4c
dev.off()
...
# Histogram overlaid with kernel density curve, fluxes by chamber
histN2Oa <- ggplot(arrayGHGdf, aes(x=N2O_nmolm2s_fluxuse)) + geom_histogram(aes(y=..density..), binwidth=.05, colour="black", fill="white") + theme_bw() + facet_wrap( ~ ChamberNameSilver, ncol = 3) + ylab("Density Distribution") + xlab("N2O Flux (nmol/m^2/s)") + theme_set(theme_bw(base_size = 36)) + xlim(c(-0.25,0.75)) +theme(axis.text.x = element_text(size=18,angle=45))
...
png(file = paste(pathsavefigs, "N2O_densityplots.png", sep=""),width=10,height=10,units="in",res=400)
histN2Oa
dev.off()
...
png(file = paste(pathsavefigs, "N2O_densityplots_combo.png", sep=""),width=5,height=5,units="in",res=400)
histN2Ob
dev.off()

# Histogram overlaid with kernel density curve, faceting by topo location
histN2Oc <- ggplot(arrayGHGdf, aes(x=N2O_nmolm2s_fluxuse)) + geom_histogram(aes(y=..density..), binwidth=.05, colour="black", fill="white") + facet_wrap(System ~ Topo, ncol = 3) + geom_density(alpha=.2, fill="#FF6666") + xlim(c(-0.5,0.75)) 
# histogram with density instead of count on y-axis
# overlay with transparent density plot
# without limiting the axes the long tail made things too hard to read

png(file = paste(pathsavefigs, "N2O_densityplots_topo.png", sep=""),width=10,height=5,units="in",res=400)
histN2Oc
dev.off()
